<script lang="ts">
  ...
  // 14px Feather-style icons (raw SVG; ContextMenu renders item.icon via @html).
  ...
  function menuItems(node: TreeNode | null): ContextMenuItem[] {
    ...
    const items: ContextMenuItem[] = [
      { label: 'New File…', icon: ICONS.newFile, action: () => beginCreate(dir, 'file') },
      { label: 'New Folder…', icon: ICONS.newFolder, action: () => beginCreate(dir, 'directory') },
    ];
    if (node) {
      items.push(
        { kind: 'separator' },
        { label: 'Rename…', icon: ICONS.rename, shortcut: 'F2', action: () => beginRename(node) },
        { label: 'Delete', icon: ICONS.delete, danger: true, action: () => deleteNode(node) },
        { kind: 'separator' },
        { label: 'Copy Path', icon: ICONS.copy, action: () => copyPath(node) },
      );
    }
    return items;
  }
