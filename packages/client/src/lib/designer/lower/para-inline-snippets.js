// @ts-nocheck — vendored transpiled Para lowering pass (see ./index.ts)
...
const innermostScope = (state) =>
  _nullishCoalesce(state.eachStack[state.eachStack.length - 1], () => state.moduleScope);
  ...
  while (i < len) {
    ...
    if (c === '(' && atBoundary) {
      ...
      if (arrow && startsJsxAt(src, arrow.bodyStart)) {
        ...
        innermostScope(state).lifted.push(
          `{#snippet ${id}(${arrow.paramsRaw})}${processedBody}{/snippet}`,
        );
